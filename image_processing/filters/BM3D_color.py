    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    for i in range(blk_num):
        for j in range(blk_num):
            ...
            m_Distance = numpy.linalg.norm((dct_img[:, :, 0] - dct_Tem_img[:, :, 0])) ** 2 / (
                    Blk_Size ** 2)  # only on luminance
...
def Aggregation_hardthreshold_color(_similar_blocks, blk_positions, m_basic_img, m_wight_img, _nonzero_num, Count,
                                    Kaiser):
    '''
...
def BM3D_1st_step_color(_noisyImg):
    ...
    for i in range(int(Width_num + 2)):
        for j in range(int(Height_num + 2)):
            ...
            Aggregation_hardthreshold_color(Similar_Blks, Positions, Basic_img, m_Wight, statis_nonzero, Count,
                                            m_Kaiser)
    for ch in range(chnl):
