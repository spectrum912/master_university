...
def processing_noise(image):
    ...
    cv2.imwrite(f"{processed_path}/noised/ASCN_10/{image}", img_noise)
...
if __name__ == '__main__':
    ...
    im_path = f"{processed_path}/filtered/Median/AWGN_10"
    POOL_SIZE = os.cpu_count() - 2

    with Executor(max_workers=POOL_SIZE) as executor:
        try:
            for r in executor.map(processing_noise, os.listdir(im_path)):
                try:
                    print(r)
    ...
    # print("start")
    #
    # im_path = f"{processed_path}/noised/ASCN_10"
    # POOL_SIZE = os.cpu_count() - 2
    #
    # with Executor(max_workers=POOL_SIZE) as executor:
    #     try:
